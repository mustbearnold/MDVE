...
The repository is public and exact candidate `87289b97e8fca3b3547eba7df229211fe40fdef9` is the immutable release commit used for this record. Its GitHub-hosted CI and CodeQL gates completed successfully ([CI run 30722441529](https://github.com/mustbearnold/MDVE/actions/runs/30722441529), [CodeQL run 30722441514](https://github.com/mustbearnold/MDVE/actions/runs/30722441514)). This candidate adds commit-bound stability evidence so the release ledger and evidence writer cannot be reused across release commits. The broader candidate includes the Codex app-server integration, generated protocol artifacts and negative initialization checks, Node 22/24 runtime gates, packaged browser/accessibility checks, browser draft reload/promotion and storage-failure checks, Recent/All/Archived/Trash library lifecycle coverage, idempotent startup, artifact checks, lifecycle checks, reliability soak, release-stability harness, dense-graph performance evidence, and Linux process-crash recovery evidence. This is technical candidate evidence, not a stable-publication decision: the manual WCAG/Orca record, registry install/signature and trusted-publishing evidence, qualified legal approval, and a separately built previous-release rollback remain release gates.

The exact `mdve-1.0.0.tgz` candidate contains 75 files and is recorded with SHA-256 `b1a7dc7b546a348e4698f670f56837bad53372aa4f141bc86d9785d94a2c590f` and SHA-512 integrity `sha512-+PwU/Jkz/Ret8QYCEr5t76rhWkygZ/AdAEVqOv01Sp54iHf4rMU6H3u4+mnNc6CAkL3B5ypU3NccBZ31X9aJ9Q==`. The 87289-bound local release-stability harness completed 10 consecutive runs, 40 suite executions, and 10,000 reliability operations without retry or skip masking; the evidence writer verified that the ledger commit equals the candidate commit. The exact-head GitHub-hosted run passed the same release matrix, including Fedora 44, Node 22/24, packaged Chromium/Firefox journeys, process-crash recovery, lifecycle retention, Codex schema, audit, reliability, and CodeQL. A redacted live Codex 0.146.0 probe on the release workstation also verified a ChatGPT account, visible model discovery, streamed new and resumed turns, interruption, and unavailable-thread rejection; this is local release-owner evidence and does not replace the remaining manual accessibility, legal, registry, or previous-stable rollback records. The raw dense TBT is retained as diagnostic evidence because Mermaid's synchronous layout is intentionally included in the measured render boundary.

Before a tag-triggered publish, the release workflow now requires the protected `npm-release` environment secret `MDVE_RELEASE_OWNER_EVIDENCE`. Its candidate-bound JSON record must contain the manual accessibility matrix, live Codex run, qualified legal approval, registry-name check, separately built previous stable, and lifecycle evidence. The workflow validates that record before `npm publish --tag next` and carries only its redacted gate summary into the release evidence bundle.
