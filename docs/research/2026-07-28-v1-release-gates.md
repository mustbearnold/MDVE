...
The repository is public and final candidate `e50ba14917659dbfd14d890d3f9a1f735698df68` completed GitHub-hosted CI and CodeQL successfully ([CI run 30702448562](https://github.com/mustbearnold/MDVE/actions/runs/30702448562), [CodeQL run 30702448569](https://github.com/mustbearnold/MDVE/actions/runs/30702448569)). The candidate adds the Codex app-server integration, generated protocol artifacts and negative initialization checks, Node 22/24 runtime gates, packaged browser/accessibility checks, browser draft reload/promotion and storage-failure checks, artifact checks, lifecycle checks, reliability soak, release-stability harness, dense-graph performance evidence, and Linux process-crash recovery evidence. This is technical candidate evidence, not a stable-publication decision: the manual WCAG/Orca record, registry install/signature and trusted-publishing evidence, qualified legal approval, a separately built previous-release rollback, and fully authenticated Codex release-owner evidence are still release gates.

The latest local Node 22.22.2/Chromium candidate run covers 20 cold and 20 warm openings of the dense 200-node/300-edge grid fixture: cold open p75/p95 569/581 ms and warm open p75/p95 641/657 ms, within the dense-render budget below. The exact-head GitHub-hosted run also passed the same dense-render budget; its raw p50/p75/p95 values are preserved in the linked workflow artifact and log. The raw dense TBT is retained as diagnostic evidence because Mermaid's synchronous layout is intentionally included in the measured render boundary.
...
The current live finding that icon-only controls expose symbols rather than descriptive names fails the app's target until repaired; a tooltip alone is not release evidence. Automated role/name assertions and a screen-reader announcement check must lock the repair in.
